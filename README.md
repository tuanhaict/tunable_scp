...
Override figure dimensions and styling directly from the CLI:

```bash
python scripts/run_experiment.py \
  --config configs/experiments/self_validation_regression.yaml \
  --figsize 12 4.5 \
  --font-size 12 \
  --title-font-size 16 \
  --label-font-size 14 \
...
The same plotting flags work with `scripts/run_suite.py` and
`scripts/make_figures.py`. Specific font-size flags override `--font-size`,
and CLI values override the corresponding `plot:` values in YAML. New runs
record the effective settings in `config.resolved.yaml`.

Use `--xlim MIN MAX` and `--ylim MIN MAX` to override Matplotlib autoscaling.
When supplied, these limits apply to every subplot generated by that command;
omit either flag to retain the previous automatic limits for that axis.
For self-validation's two different panels, use `--coverage-xlim` and
`--coverage-ylim` for the coverage panel, or `--size-xlim` and `--size-ylim`
for the average-set-size panel. Panel-specific limits override global limits.
...
For example, change only the presentation without rerunning the experiment:

```bash
python scripts/make_figures.py \
  --run-dir outputs/self_validation_regression/<timestamp> \
  --figsize 14 4.5 --title-font-size 17 --label-font-size 14 --dpi 300
```
